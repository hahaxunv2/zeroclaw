...
3. **Implement minimal patch** — apply KISS/YAGNI/DRY rule-of-three; no speculative abstractions or config keys without a concrete use case.
4. **Validate by risk tier** — docs-only: lightweight checks. Code changes: full relevant checks.
5. **Document impact** — update PR notes for behavior, risk, side effects, and rollback. Update corresponding runtime-contract references and keep supported locale README/docs-hub navigation aligned. Follow `docs/i18n-guide.md` for completion.
6. **Queue hygiene** — stacked PR: declare `Depends on #...`. Replacing old PR: declare `Supersedes #...`. Clean up task branches/worktrees after merge/close.
